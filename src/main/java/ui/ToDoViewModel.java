...
public class ToDoViewModel extends JFrame{
    ...
    public ToDoViewModel(User user) {
        ...
        for (Category cat: categories) { // loops through all the user's categories
            ...
            this.add(new CategoryList(cat.getTasks().getTasks().size(), cat.getTasks().convertToArray()), BorderLayout.CENTER);  // ? need getter for getH_items (hashmap) from TaskFactory, size = num of tasks in hasmap
